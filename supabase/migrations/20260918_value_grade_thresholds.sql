-- Run after 20260918_shopping.sql. Update grades, preserving amounts, prices and nutrition snapshots.
begin;
...
-- Regrade existing history from its saved percentages, not current targets/catalogue.
update public.shopping_trip_items i set overall_grade = case
 when i.calorie_percentage is null or i.protein_percentage is null then null
 when (least(i.calorie_percentage,100)+least(i.protein_percentage,100))/2 >= 90 then 'A'
 when (least(i.calorie_percentage,100)+least(i.protein_percentage,100))/2 >= 70 then 'B'
 when (least(i.calorie_percentage,100)+least(i.protein_percentage,100))/2 >= 50 then 'C'
 when (least(i.calorie_percentage,100)+least(i.protein_percentage,100))/2 >= 30 then 'D'
 else 'E' end;
commit;
